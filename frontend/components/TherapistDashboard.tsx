...
const TherapistDashboard: React.FC = () => {
  ...
  const fetchPatientData = async () => {
    try {
      ...
      const response = await axios.get('http://127.0.0.1:5000/summarize_journal'); 
      console.log("debug 1")
      ...
      const fetchedPatient: Patient = {
        ...
        status: 'Active',
        mentalHealthMetrics: {
          anxiety: data.chartData.map((item: any) => item.Anxiety), 
          happiness: data.chartData.map((item: any) => item.Happiness), 
        },
        summaryText: data.summaries.join(' '), 
        scores: data.scores,
      };
